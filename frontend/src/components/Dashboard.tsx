...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchPicks = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/picks?region=${region}&timeframe=${timeframe}`);
  ...
  return (
    <div className="flex flex-col flex-1 p-8 text-white max-w-[90rem] mx-auto w-full relative z-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 backdrop-blur-md bg-white/5 border border-white/10 p-6 rounded-2xl shadow-2xl gap-6">
        <div>
          <h1 className="text-4xl font-extrabold bg-gradient-to-r from-cyan-400 to-blue-600 bg-clip-text text-transparent">
            AI Pre-Market Momentum
          </h1>
          ...
          <div className="flex bg-zinc-900/80 p-1 rounded-xl border border-white/10">
            {['ALL', 'INDIA', 'WORLD'].map(r => (
              <button 
                key={r}
                onClick={() => setRegion(r)}
                className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg transition-all ${region === r ? 'bg-cyan-500/20 text-cyan-400 shadow-md' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
              >
          ...
          <div className="flex bg-zinc-900/80 p-1 rounded-xl border border-white/10">
            {['1D', '1W', '1M', '7W'].map(t => (
              <button 
                ...
                className={`flex items-center gap-1 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${timeframe === t ? 'bg-blue-500/20 text-blue-400 shadow-md' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
              >
